...
def main():
	...
	df = pd.read_csv("Yaxche_Territories.csv")
	...
	small_df = small_df[["Restless Shores", "Monarch's Bluffs", "Reekwater", "Weaver's Fen", "Cutlass Keys", "Mourningdale", "First Light"]] #"Brightwood", "Everfall", "Windsward", "Ebonscale Reach"]]
	print(small_df.head())
	ax = small_df.plot(ylim=(-30000,200000), lw=2, marker='.', markersize=10, title='Profits by Territory', grid=True)
	# ax.set(xlabel="Week", ylabel="Profit in Coin")
	ax.set_ylabel("Profit in Coin", labelpad=0)
	ax.set_xlabel("Week")
	plt.legend(ncol=3)
	ax.get_figure().savefig("Lower_Profits.png")
	return 0
